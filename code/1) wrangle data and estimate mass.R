...
emergence_2019_new <- read_excel("data/emergence_2019_new.xlsx") %>% select(anisoptera, canacidae, cerat, 
                                                                            ...
                                                                            zygoptera,
                                                                            start, collect, location) %>%
  group_by(start, collect, location) %>% 
  mutate(id = row_number()) %>% 
  pivot_longer(cols = c(-start, -collect, -location, -id)) %>% 
  mutate(julian = yday(collect),
         trap_days = yday(collect) - yday(start)) 

saveRDS(emergence_2019_new, file = "data/emergence_2019_new.rds")

#get taxized list from fish database
prey_taxa_have <- read_csv("data/prey_taxa_have.csv") %>% 
...
# bring in data with image names and filter to select only sample_id's with images and only taxa with + abundance
# NEW_diet2019_fixed_images <- read_excel("data/NEW_diet2019_fixed_images.xlsx") %>% 
#   filter(!is.na(pic_1)) %>%
#   type_convert() %>% 
...
#   filter(value > 0) 
# 
# write.csv(NEW_diet2019_fixed_images, file = "data/NEW_diet2019_fixed_images_enter.csv", row.names = F)

# drive_upload("data/NEW_diet2019_fixed_images_enter.csv", 
#              path = "abe/NEW_diet2019_fixed_images_enter.csv",
#              type = "spreadsheet",
#              overwrite = T)

#download length data from google drive

# drive_download("abe/NEW_diet2019_fixed_images_enter",
#                path = "data/diet2019_with_lengths.csv", overwrite = T)
...
diet2019_with_lengths <- read_csv("data/diet2019_with_lengths.csv") %>% 
  rename(prey_type = name)
...
length_mass_equations <- read_csv("data/length_mass_regressions.csv") %>% 
...
mass_data_bowcreek <- read_excel("data/Bow Creek Insect weights.xlsx") %>% clean_names() %>%
...
# saveRDS(bow_taxized, file = "data/bow_taxized.rds")
bow_taxized <- readRDS("data/bow_taxized.rds")
...
feeding_categories <- read_csv("data/feeding_categories.csv") %>% select(prey_taxon, prey_feeding, prey_stage)
...
fish_taxa <- readRDS("data/guild_diet_multi.rds") %>% distinct(fish, species, finalguild) %>% 
