...
prop_nonfeed_aquatic <- guild_diet_multi_drymass %>% filter(prey_ecosystem == "aquatic") %>% 
  ...
  group_by(fish_species, prey_feeding, sample_id) %>% 
...
# emergence -----------------
emergence <-read_excel("data/emergence_2019_new.xlsx") %>% select(anisoptera, canacidae, cerat, 
...
# Range of emergence
emergence %>%
  mutate(anisoptera = as.numeric(anisoptera)) %>% 
  replace(is.na(.), 0) %>%
  mutate(sum = rowSums(across(where(is.numeric))),
         prop_chiro = chiro/sum) %>% 
  select(sum) %>% 
  View()


# What proportion of emergence is chironomids?
emerge_chiroprop <- emergence %>%
  mutate(anisoptera = as.numeric(anisoptera)) %>% 
  replace(is.na(.), 0) %>%
  mutate(sum = rowSums(across(where(is.numeric))),
         prop_chiro = chiro/sum) 
...
emerge_chiroprop %>% 
  filter(sum != 0) %>% 
  summarize(mean = mean(prop_chiro, na.rm = T),
            sd = sd(prop_chiro, na.rm = T),
            total_emergence = sum(sum))

emerge_chiroprop %>% 
  ggplot(aes(x = julian(collect), y = prop_chiro, size = sum)) +
