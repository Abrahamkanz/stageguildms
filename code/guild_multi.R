...
guild_diet_multi <- readRDS(file = "data/guild_diet_multi_drymass.rds")


guild_chiro_multi <- guild_diet_multi %>% 
  filter(grepl("hironomid", prey_taxa)) %>% 
  pivot_wider(names_from = prey_stage, values_from = number) %>% 
  mutate(date = as.factor(date),
         a = as.integer(a),
         l = as.integer(l),
         p = as.integer(p))

saveRDS(guild_chiro_multi, "data/guild_chiro_multi.rds")

guild_brm <- brm(mvbind(a,l,p) ~ date*finalguild + (1|site) + (1|species), 
                 data = guild_chiro_multi, family = poisson(link = "log"),
